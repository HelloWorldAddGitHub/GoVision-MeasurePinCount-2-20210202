...
namespace GoVision
{
    public partial class AutoForm : Form
    {
        ...
        public void ChangeProduct(string name, string path)
        {
            ...
            ((PinCameraForm)fainForm)?.ChangeProduct(name, path);
        }
        ...
        public bool ChangeUser(UserMode user, string password)
        {
            if (user == UserMode.Manager && password == Param.ManagerPassword)
            {
                ...
                m_dicForm[btnSideCameraA].Enabled = true;
            ...
            if (user == UserMode.Operator && password == Param.OperatorPassword)
            {
                ...
                m_dicForm[btnSideCameraA].Enabled = false;
        ...
        private void AutoForm_Load(object sender, EventArgs e)
        {
            ...
            tlbVer.Text = $"V2.2：{System.IO.File.GetLastWriteTime(this.GetType().Assembly.Location)}";
            ...
            CameraName.SideCameraA = IniTool.GetString(fileName, "CameraName", "SideCameraA", "HE012A1GMA");
            CameraName.SideCameraB = IniTool.GetString(fileName, "CameraName", "SideCameraB", "HE012A1GMB");
            ...
            VisionMgr.GetInstance().AddCamera(new CameraMVision(CameraName.SideCameraA));
            VisionMgr.GetInstance().AddCamera(new CameraMVision(CameraName.SideCameraB));
            //VisionMgr.GetInstance().AddCamera(new CameraGige(CameraName.MainCamera));
            //VisionMgr.GetInstance().AddCamera(new CameraGige(CameraName.SideCameraA));
            //VisionMgr.GetInstance().AddCamera(new CameraGige(CameraName.SideCameraB));
#endif
            ...
            VisionMgr.GetInstance().AddVisionStep(CameraName.SideCameraB, new ProcessSideMea(VisionStepName.SideMeaB));
            ...
            //关联页面和按钮
            m_dicForm.Add(btnMainCamera, new PinCameraForm());
            ...
            m_dicForm.Add(btnSideCameraA, new SideCameraForm(CameraName.SideCameraA, VisionStepName.SideMeaA));
            m_dicForm.Add(btnSideCameraB, new SideCameraForm(CameraName.SideCameraB, VisionStepName.SideMeaB));
            ...
            Log.Show = ((PinCameraForm)m_dicForm[btnMainCamera]).ShowLog;
            Data.Show = ((PinCameraForm)m_dicForm[btnMainCamera]).ShowData;
        ...
        private void AutoForm_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            IniTool.Set(fileName, "CameraName", "SideCameraA", CameraName.SideCameraA);
            IniTool.Set(fileName, "CameraName", "SideCameraB", CameraName.SideCameraB);
        ...
        private void btnSideCameraRight_Click(object sender, EventArgs e)
        {
            SwitchWnd(btnSideCameraB);
        }
    }
}